...
DEFAULT_WATCHLIST = [
    '^FCHI', '^SBF120', 'ML.PA', 'FP.PA', 'SAN.PA', 'OR.PA',
    'BNP.PA', 'AI.PA', 'SU.PA', 'RMS.PA', 'KER.PA', 'CAP.PA'
]
...
def get_currency(symbol):
    ...
    if exchange in ['Euronext Paris', 'Euronext Amsterdam']:
        return 'EUR'
    elif exchange == 'Indice':
        return 'EUR'
    else:
        return 'USD'
...
def get_trading_data(symbol):
    try:
        ...
        for period, interval in periods.items():
            try:
                ...
                if hist.index.tz is None:
                    hist.index = hist.index.tz_localize('UTC').tz_convert(PARIS_TIMEZONE)
                else:
                    hist.index = hist.index.tz_convert(PARIS_TIMEZONE)
                ...
                # Construction des bougies avec gestion d'erreur
                candles = []
        ...
        # Nettoyage final et mise en cache
...
# ============================================================
# ROUTE TOP PERFORMERS CORRIGÉE
# ============================================================

@app.route('/api/top-performers')
def get_top_performers():
    try:
        symbols = ['ML.PA', 'FP.PA', 'SAN.PA', 'BNP.PA', 'OR.PA',
                  'AI.PA', 'SU.PA', 'RMS.PA', 'CAP.PA', 'KER.PA']
...
def get_market_status():
    try:
        ...
        hour = paris_now.hour
        minute = paris_now.minute
        weekday = paris_now.weekday()
        ...
        is_open = (hour >= 9 and (hour < 17 or (hour == 17 and minute <= 30)))

        return jsonify({
            'status': 'open' if is_open else 'closed',
            'label': 'Ouvert' if is_open else 'Fermé',
            'icon': '🟢' if is_open else '🔴',
            'time': paris_now.strftime('%H:%M:%S')
        })
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("🖥️  MINITEL BOURSE - Version Corrigée")
    print("=" * 60)
    print(f"🌐 http://localhost:5000")
    print(f"📊 Cache activé ({CACHE_DURATION}s)")
    print(f"🔧 Debug: {app.debug}")
    print("=" * 60)
    print("💡 Pour vider le cache: http://localhost:5000/api/clear-cache")
    print("=" * 60)
